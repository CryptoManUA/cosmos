...
function main_menu {
    while true; do
		...
		9)
			...
			echo "↓ Вкажіть кількість токенів ↓:"
			read AMOUNT
            lavad tx staking delegate $(lavad keys show wallet --bech val -a) $AMOUNTulava --from wallet --chain-id lava-testnet-2 --gas-prices 0.1ulava --gas-adjustment 1.5 --gas auto -y && sleep 2
            echo ""
			;;
		10)
            echo ""
