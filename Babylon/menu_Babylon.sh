...
function main_menu {
	...
	clear
		...
		9)
			...
			echo "↓ Вкажіть кількість токенів ↓:"
			read AMOUNT
            babylond tx epoching delegate $(babylond keys show wallet --bech val -a) "$AMOUNT"ubbn --from wallet --chain-id bbn-test-2 --gas-prices 0.1ubbn --gas-adjustment 1.5 --gas auto -y && sleep 2
            echo ""
			;;
		10)
            echo ""
