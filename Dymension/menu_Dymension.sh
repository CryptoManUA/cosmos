...
function main_menu {
    while true; do
		...
		9)
			...
			echo "↓ Вкажіть кількість токенів ↓:"
			read AMOUNT
            dymd tx staking delegate $(dymd keys show wallet --bech val -a) "$AMOUNT"udym --from wallet --chain-id froopyland_100-1 --gas-prices 0.1udym --gas-adjustment 1.5 -y && sleep 2
            echo ""
			;;
		10)
            echo ""
